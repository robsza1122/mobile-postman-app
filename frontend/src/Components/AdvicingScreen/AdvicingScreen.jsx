...
import { placeOfAdvice, placeOfNotification, reasonOfAdvice } from "../../utils/DataProvider";
...
export const AdvicingScreen = () => {
    ...
    const {setDownloadedBook, downloadedBook, currentParcels} = useContext(PostManState);
    const {mutate: adviceParcel} = useMutation({
        mutationKey: ["advicedParcel"],
        mutationFn: addAdvicedStatus,
    })
    const [chooseReason, setChooseReason] = useState('No one at home / closed company');
    const [showReason, setShowReason] = useState(false);
    const [showOffice, setShowOffice] = useState(false);
    const [showNotifiedPlace, setShowNotifiedPlace] = useState(false);
    const [chooseNotifiedPlace, setChooseNotifiedPlace] = useState('Post box of addressee');
    ...
    return (
        ...
        {showReason && (
            <>
            <div className="advice__background">
                
            </div>
            <div className="advice__window">
            {reasonOfAdvice.map((reason) => (
            <>
                    <button className="advice__button" onClick={() => onChooseReason(reason)}>{reason}</button>
                    </>
            ))} 
            </div>
            </>
        ...
        {showOffice && (
            ...
            <>
                    <button className="advice__button" onClick={() => onChooseOffice(reason)}>{reason}</button>
                    </>
            ))} 
            </div>
            </>
        ...
        {showNotifiedPlace && (
            ...
            <>
                    <button className="advice__button" onClick={() => onChooseNotificationPlace(reason)}>{reason}</button>
                    </>
            ))} 
            </div>
            </>
...
<nav className="advice__nav">
      ...
      <div className="deliver__position" key={currentParcels[0]._id} style={{ 
        width: '100%',
        backgroundColor: 'white',
        }}>
              <div className="deliver__positioncontent">
                <p className="deliver__number">{currentParcels[0].numberOfParcel}</p>
      ...
      <div className="advice__content">
        ...
        <button className="advice__selecttext" onClick={() => onReason()}>{chooseReason}</button>
        <p className="advice__text">Post Office:</p>
        <button className="advice__selecttext" onClick={() => onOffice()}>{chooseOffice}</button>
        <p className="advice__text">Place of leaving notification:</p>
        <button className="advice__selecttext" onClick={() => onPlaceNotification()}>{chooseNotifiedPlace}</button>
      </div>
