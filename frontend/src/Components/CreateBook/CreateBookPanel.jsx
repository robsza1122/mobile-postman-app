...
export const CreateBookPanel = ({ assignedUser, setShowUsersWindow }) => {
  ...
  const handleTogglingParcels = () => {
    const allAreChecked =
      parcels.filter((parcel) => parcel.isMarked && !parcel.isBooked).length ===
      parcels.filter((parcel) => !parcel.isBooked).length;
