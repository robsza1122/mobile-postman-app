...
export const WorkPage = () => {
  ...
  const {slideOptions, downloadedBook, currentUser, settled} = useContext(PostManState);
  console.log(downloadedBook); 
  const parcelsToDeliver = parcels.filter(parcel => parcel.status[parcel.status.length - 1].name === "IN DELIVERY" && parcel.forUser === currentUser.username)
const advicedParcels = parcels.filter(parcel => parcel.status[parcel.status.length - 1].name === "ADVICED" && parcel.forUser === currentUser.username)
const otherParcels = parcels.filter(parcel => parcel.status[parcel.status.length - 1].name === "OTHER" && parcel.forUser === currentUser.username);
  ...
  const myParcelsOptions = [
    {id: 1, header: "SHOW ALL", amount: currentUser.parcels.length, title: "DOWNLOADED"},
